...
## 一、实验知识点与对应OS原理关联分析
| 实验知识点                | 对应OS原理知识点                          | 理解与关联分析                                                                 |
|---------------------------|-------------------------------------------|------------------------------------------------------------------------------|
| 1. 进程控制块（PCB）初始化 | 进程管理 - PCB结构与作用                  | - 实验中`proc_struct`包含进程状态、PID、上下文等核心字段，是OS管理进程的核心数据结构；<br>- 原理中PCB是进程存在的唯一标识，OS通过PCB实现对进程的创建、调度、终止等全生命周期管理；<br>- 实验通过`alloc_proc`初始化PCB，体现了OS为进程分配资源的第一步——建立管理元数据。 |
| 2. 内核栈分配（`setup_kstack`） | 进程的内存布局 - 内核栈与用户栈分离        | - 实验中为每个内核线程分配独立内核栈，用于存储中断/异常处理时的寄存器上下文和函数调用栈；<br>- 原理中内核栈是进程在内核态运行时的内存空间，与用户栈隔离可防止用户态代码破坏内核数据；<br>- 实验通过`kstack`字段记录栈基址，体现了OS对进程内存资源的精细化分配。 |
| 3. 进程上下文切换（`switch_to`） | 进程调度 - 上下文切换机制                  | - 实验中`switch_to`通过保存/恢复`context`中的寄存器（ra、sp、s0~s11），实现内核线程间的执行流切换；<br>- 原理中上下文切换是调度器的核心操作，需保存当前进程的CPU状态（寄存器、PC等），恢复目标进程状态；<br>- 实验中`context`仅保存被调用者保存寄存器，符合RISC-V函数调用约定，优化了切换开销，体现OS设计的效率考量。 |
| 4. 进程创建（`do_fork`）   | 进程创建 - fork系统调用原理                | - 实验中`do_fork`通过复制PCB、内核栈、trapframe等资源，创建当前进程的副本；<br>- 原理中fork是OS创建新进程的核心接口，通过“写时复制”（COW）优化资源复制效率（本实验未实现COW，直接复制）；<br>- 实验中`copy_thread`设置子进程trapframe和context，确保子进程从正确的入口执行，体现了OS对进程初始化的完整性保障。 |
| 5. 中断开关（`local_intr_save`/`local_intr_restore`） | 中断管理 - 临界区保护                     | - 实验中通过禁用/启用中断，保护上下文切换等临界区操作不被中断打断；<br>- 原理中临界区是OS内核中需要原子执行的代码段，中断是导致并发问题的主要原因之一；<br>- 实验通过保存中断状态（`intr_flag`），实现临界区保护的同时不影响其他场景的中断响应，体现OS对并发控制的严谨性。 |
| 6. 页表切换（`lcr3`）      | 内存管理 - 地址翻译与页表机制              | - 实验中内核线程共享`boot_pgdir_pa`页表，无需切换；若为用户进程则需切换页表实现地址隔离；<br>- 原理中页表是虚拟地址到物理地址的映射表，OS通过切换页表让不同进程看到独立的地址空间；<br>- 实验中`pgdir`字段记录页表基址，体现了OS对内存地址空间的管理核心——页表控制。 |


## 二、OS原理重要但实验未覆盖的知识点
| 知识点分类                | 具体知识点                          | 未覆盖原因分析                                                                 |
|---------------------------|-------------------------------------|------------------------------------------------------------------------------|
| 1. 进程调度算法            | 时间片轮转、优先级调度、多级反馈队列 | 实验中仅实现简单的调度逻辑（`idleproc`循环检查`need_resched`），未涉及复杂调度算法；核心目标是验证进程创建与切换，而非调度策略优化。 |
| 2. 内存管理 - 分页进阶     | 写时复制（COW）、页面置换算法（LRU） | 实验中进程创建时直接复制内核栈和trapframe，未实现COW优化；无用户态进程，无需页面置换（页面置换是解决内存不足时的策略）。 |
| 3. 进程同步与通信          | 信号量、互斥锁、管道、消息队列       | 实验中仅涉及单CPU下的内核线程，无并发访问共享资源的场景，无需同步机制；无多进程间数据交换需求，未实现进程通信接口。 |
| 4. 中断与异常处理细节      | 中断向量表、异常类型分类（如缺页异常） | 实验中仅使用`local_intr_save`/`local_intr_restore`开关中断，未涉及中断的注册、分发流程；无用户态进程，缺页异常等常见异常场景未触发。 |
| 5. 文件系统                | 文件描述符、目录结构、磁盘I/O        | 实验聚焦于进程管理和内存管理的核心机制，未涉及文件系统模块；ucore后续实验会逐步引入文件系统相关功能。 |
| 6. 系统调用                | 系统调用接口、陷入（trap）机制        | 实验中内核线程直接在内核态运行，无需通过系统调用切换到内核态；系统调用是用户态进程与内核交互的桥梁，本实验未涉及用户态。 |
| 7. 多处理器调度            | SMP架构下的调度同步、CPU亲和性        | 实验基于单CPU环境，未涉及多处理器下的进程调度竞争问题；多处理器调度需解决缓存一致性、锁竞争等复杂问题。 |
| 8. 死锁检测与避免          | 死锁必要条件、银行家算法              | 实验中无多进程竞争资源的场景，不存在死锁风险；死锁是多进程并发时的重要问题，需OS通过算法检测或避免。 |
