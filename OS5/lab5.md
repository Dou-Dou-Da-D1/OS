...
```c
// COW 版本的 copy_range (设计,未在当前代码中实现)
if (share || (perm & PTE_W)) {
    // 增加引用计数
    page_ref_inc(page);
...
1. **内核态操作**:
   - **fork**: 分配进程控制块、复制内存空间、建立进程关系、分配PID
   - **exec**: 释放旧内存空间、解析ELF文件、建立新内存映射、设置trapframe
   - **wait**: 查找僵尸子进程、进程休眠与唤醒、回收进程资源
   - **exit**: 释放内存资源、设置僵尸状态、过继子进程、唤醒父进程

#### 用户态与内核态的交错执行流程

以一个完整的 **fork + exec + wait + exit** 流程为例:
...
T1   [用户态] 执行 main()
         |
T2   [用户态] 调用 fork() -> ecall
         |
T3   [内核态] 异常处理: __alltraps
         |           trap() -> syscall()
         |           -> sys_fork() -> do_fork()
         |           创建子进程,复制资源
         |           设置子进程 trapframe.a0 = 0
         |           父进程 trapframe.a0 = child_pid
         |
T4   [内核态] sret 返回用户态
         |
T5   [用户态] fork() 返回 child_pid      [用户态] fork() 返回 0
         |    判断 pid > 0                    |    判断 pid == 0
         |                                     |
T6   [用户态] 调用 wait() -> ecall       [用户态] 调用 exec() -> ecall
         |                                     |
T7   [内核态] do_wait()                   [内核态] do_execve()
         |    查找子进程状态                  |    释放旧内存
         |    未找到 ZOMBIE                   |    load_icode()
         |    -> PROC_SLEEPING                |    设置新 trapframe
         |    -> schedule() 切换              |    sret 返回
         |                                     |
         |                                [用户态] 新程序开始执行
         |                                     |    工作完成
...
用户程序: fork();
    ↓
用户库函数: 设置寄存器(a0=系统调用号, a1-a5=参数)
    ↓
执行: ecall 指令
    ↓
硬件响应: 
  - 保存 PC 到 sepc (保存返回地址)
  - 设置 sstatus.SPP = 0 (记录来自用户态)
  - PC = stvec (跳转到异常处理入口)
  - 切换到内核态
    ↓
软件处理: __alltraps
  - 保存完整的 trapframe (所有寄存器)
  - 调用 trap() → syscall() → sys_fork()
```
...
内核: sys_fork() 执行完成
    ↓
返回值处理: tf->gpr.a0 = 返回值 (写入trapframe)
    ↓
返回路径: syscall() → trap() → __trapret
    ↓
__trapret:
  - 从 trapframe 恢复所有寄存器
  - 特别恢复: a0 (包含返回值)
...
执行: sret 指令
    ↓
硬件响应:
  - PC = sepc (返回到 ecall 的下一条指令)
  - 根据 sstatus.SPP 切换到用户态
  - 清除 sstatus.SPP
    ↓
用户程序: 从 fork() 调用处继续执行
  - 获取返回值 (已在 a0 寄存器中)
...
1. **寄存器约定**:
   - RISC-V 使用 `a0` 寄存器传递系统调用返回值
   - 内核在 `syscall()` 中设置: `tf->gpr.a0 = syscalls[num](arg);`

2. **具体示例 - fork 的两个返回值**:

```c
// 在 do_fork() 中
子进程的trapframe: proc->tf->gpr.a0 = 0;        // 子进程返回0
父进程的trapframe: current->tf->gpr.a0 = child_pid;  // 父进程返回子进程PID
```
...
void syscall(void) {
    ...
    // 执行系统调用并将返回值写入 a0
    tf->gpr.a0 = syscalls[num](arg);  // 关键: 返回值写入trapframe
    
    // 返回后,__trapret 会恢复 a0 的值到寄存器
}
```
...
**总结**: 内核通过修改保存在 trapframe 中的 `a0` 寄存器值来返回结果,当 `sret` 指令恢复寄存器状态时,用户程序自然获得返回值。

---

### 3.3 问题2: 用户态进程执行状态生命周期图
...
```
  ...
  选中调度   │    │
      ...
      └──────────────┘
             ...
             ├─────────┬──────────┬───────────┬──────────┐
             │         │          │           │          │
             │ do_wait │ do_sleep │ 时间片用完│  异常    │ do_exit
             │         │          │ yield()   │          │ (进程终止)
             ▼         ▼          │           │          ▼
      ┌──────────────┐            │           │   ┌─────────────┐
...
1. PROC_UNINIT → PROC_RUNNABLE
   触发事件: wakeup_proc(proc)
   发生时机: do_fork() 创建进程完成时
...
3. RUNNING → PROC_RUNNABLE
   触发事件: 
     - 时间片用完: ticks % TICK_NUM == 0, current->need_resched = 1
     - 主动让出CPU: do_yield()
   发生时机: 时钟中断或系统调用
   关键函数: schedule()
...
4. RUNNING → PROC_SLEEPING
   触发事件: 
     ...
     - do_sleep(): 主动休眠
   发生时机: 系统调用
   关键函数: current->state = PROC_SLEEPING; schedule()
...
5. PROC_SLEEPING → PROC_RUNNABLE
   ...
   发生时机: 
     - 子进程退出时唤醒父进程
     - 其他同步事件
   关键函数: wakeup_proc(proc)

6. RUNNING → PROC_ZOMBIE
   触发事件: do_exit(error_code)
   发生时机: 
     - 用户程序调用 exit()
     - 程序异常终止
   关键函数: do_exit()
...
- RUNNING 不是独立的进程状态标志,而是 PROC_RUNNABLE 进程
  被调度器选中后占用 CPU 的执行状态

- PROC_ZOMBIE 进程不会被调度执行,只等待父进程回收资源

- 如果父进程先于子进程退出,子进程会被过继给 initproc

- 孤儿进程(父进程已退出的进程)由 init 进程负责回收
```
...
Value returned is $22 = 0  # 返回值 0 表示翻译成功
```
...
1. **Sv39 三级页表机制验证**:
   - 成功观察到 QEMU 从 satp 寄存器读取页表基址 `0x80204000`
   - 虚拟地址 `0xffffffffc0203ff8` 被正确拆分为 VPN[2]=511, VPN[1]=1, VPN[0]=3, Offset=0xff8
   - 第一级页表项 `0x200000cf` 的 R/W 位置位，表明这是一个 2MB 大页映射

2. **大页映射优化**:
   - ucore 内核使用大页映射减少 TLB 缺失
   - 一个大页可以映射 2MB 空间 (512 个 4KB 页)
   - 减少了页表级数，提高了地址翻译效率

3. **直接映射区域**:
   - 虚拟地址 `0xffffffffc0000000 ~ 0xffffffffc0400000` (内核空间)
   - 直接映射到物理地址 `0x80000000 ~ 0x80400000` (DRAM 起始位置)
   - 映射关系: VA - 0xffffffff40000000 = PA

#### 双重 GDB 调试技术总结

**调试层次**:
```
Layer 3: ucore 内核 GDB (终端3)
         ↓ 单步执行内核代码
Layer 2: QEMU GDB stub (终端1)
         ↓ 模拟 RISC-V 指令执行
Layer 1: QEMU 进程 GDB (终端2)
         ↓ 调试 QEMU 虚拟化逻辑
Layer 0: x86 物理 CPU
```

**优势**:
- **完整的可见性**: 从用户视角 (内核代码) 到虚拟化实现 (QEMU 源码) 全程可追踪
- **双重验证**: 可以对比 ucore 的预期行为与 QEMU 的实际模拟结果，发现潜在 bug
- **深入理解**: 理解硬件地址翻译机制如何被软件模拟

**局限性**:
- **调试复杂性**: 需要在三个终端间切换，容易混淆上下文
- **性能开销**: QEMU 进程被 GDB 暂停时，ucore 无法继续执行
- **符号精度**: GDB 打印 QEMU 内部变量时可能出现精度问题
